...
class ContentExtractor(object):
    ...
    def get_authors(self, doc):
        ...
        def parse_byline(search_str):
            ...
            search_str = hangul.sub('', search_str)
        ...
        ATTRS_M = ['name', 'rel', 'itemprop', 'class', 'id', 'content', 'property']
        VALS_M = ['author', 'dc.creator', 'dable:author']
        ...
        for match in matches:
            ...
            #영어제거
            hangul = re.compile('[^ ㄱ-ㅣ가-힣]+')
            possible_txt = hangul.sub('', possible_txt)
            # 너무 긴 텍스트는 기자 이름이 아닐 가능성이 높다
            if len(possible_txt) < 8 :
